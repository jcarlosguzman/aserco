...
#Este programa agregar tipo de insumo
#
#
#Id: agregar_tipo.php Ver. 1.0
#
# Control de Cambios:
...
<head>
  ...
  <title>Agregar Tipo de Insumo | AdminPanel | <?php echo $CFG->empresa;?></title>
...
?>
		...
		<section class="content-header">
		  <ol class="breadcrumb">
            ...
            <li class="active">Agregar Tipo de Insumo</li>
		...
		<section class="content">
		  ...
		  <div class="box">
			<div class="box-header with-border">
			  <h2 class="box-title">Agregar Tipo de Insumo</h2>
...
<?php
   ...
   // include the Zebra_Form class
	 ...
	 $form->clientside_validation(array(
        ...
        $obj = $form->add('text', 'txt_Tipo','',array('class' => 'form-control','style' => 'width:90% !important;','aria-required' => true));
		$obj->set_rule(array(
                'required'  => array('error', 'El campo "Tipo de Insumo" es obligatorio')
             ));		
		    
 
        // Abreviatura
        $form->add('label', 'lbl_Abreviatura', 'txt_Abreviatura', 'Abreviatura :');
        $obj = $form->add('text', 'txt_Abreviatura','',array('class' => 'form-control','style' => 'width:90% !important;'));
		
		$obj->set_rule(array(
                'required'  => array('error', 'El campo "Abreviatura" es obligatorio'),
	    ...
	    $obj = $form->add('radios', 'opt_Estado', array(
	        'A' =>  'Activo',
	        'I' =>  'Inactivo',
	    ),'',array('style' => 'width:30px;','aria-required' => true));
	    $obj->set_rule(array(
	        'required' => array('error', 'El campo "Estatus" es obligatorio')
	    ));
        ...
        $form->add('submit', 'btnsubmit', 'Grabar ',array('class' =>'btn btn-success'));
		$form->add('button', 'btnback', 'Regresar ','button',array('class' =>'btn btn-success','onclick'=>'javascript:window.location = "'. $CFG->url_admin . '/modulos/mod_tipos_insumo_v1.0/listado.php"'));
        ...
        if ($form->validate()) {
			...
			$qry="insert into tb_tipo_insumo set
					  tipo_insumo=:tipo,
					  estado=:estado,
			   ...
			   //Se hace bind a los datos a instertar
			   $database->bind(':tipo',htmlspecialchars_decode(trim($_POST['txt_Tipo'])),PDO::PARAM_STR);
			   $database->bind(':estado',$_POST['opt_Estado'],PDO::PARAM_STR);		
			   $database->bind(':abreviatura',htmlspecialchars_decode(trim($_POST['txt_Abreviatura'])),PDO::PARAM_STR);
			   ...
			   if ($database->execute()) {
					...
					$id_contenido=$database->lastInsertId();
					$tipo_contenido="tipo_insumo";
					$url=urls_amigables($_POST['txt_Tipo']);
					   
   
					$nodo=agrega_nodo($id_contenido,$tipo_contenido,$url);
                    
					//Actualizando en numero del nodo en el contenido generado
					$qry="update tb_tipo_insumo set nodo=$nodo where id_tipo=$id_contenido";
			        $database->query($qry);   //se prepara el query
					$database->execute();
					...
					echo "<div class=\"row margin-vertical-20\">
                             <div class=\"col-md-5 col-md-offset-3\">
                                  <div class=\"alert alert-success\" role=\"alert\">
								      <span class=\"h3\"><span class=\"glyphicon glyphicon-ok\" aria-hidden=\"true\"></span><strong> {$msgs['grabado_exito']}</strong><br />
									  <div id=\"datos_nodo\"><span class=\"negritas\">Nodo: </span>$nodo<br />
								           <span class=\"negritas\">URL: </span> /$tipo_contenido/$url<br />
								      </div>
								   </div>
                             </div>
                          </div>";
					...
					echo "<div class=\"row margin-vertical-20\">
                             <div class=\"col-md-5 col-md-offset-3\">
    	                        <div class=\"alert alert-danger\" role=\"alert\">
	 		                        <span class=\"h3\"><span class=\"glyphicon glyphicon-remove\" aria-hidden=\"true\"> </span><strong>  {$msgs['grabado_error']}</strong></span>
  		                        </div>
                             </div>
                          </div>";
				 }
...
<div id="load"><img src="<?php echo $CFG->url_admin;?>/imagenes/ajax_loader.gif"/></div>
</div>
...
$("#form-elementos").submit(function(e){
	...
	if ($("#form-elementos").data('Zebra_Form').validate()) {
